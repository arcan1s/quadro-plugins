...
{
    QProcess::startDetached(m_configuration["Action"].toString());
}
...
void DesktopEngine::update()
{
    ...
    m_process->start(m_configuration["Executable"].toString());
}
...
void DesktopEngine::updateData(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    m_data = QTextCodec::codecForMib(106)
...
{
    return QSize(m_configuration["Width"].toInt(),
                 m_configuration["Height"].toInt());
}
