...
FavLauncher::~FavLauncher()
{
    ...
    for (auto &plugin : m_plugins) {
        ...
        QString name = plugin["Plugin"].toString();
...
void FavLauncher::init()
{
    ...
    QStringList plugins
        = {"datetime", "pcstatus", "newsplugin", "pcstatus", "yahooweather"};
...
void FavLauncher::setArgs(QuadroCore *core)
{
    ...
    // plugin container
    m_container = new PluginContainer(
        widget, m_core->config()->property("GridSize").toInt(), QString(),
        Qt::ScrollBarAlwaysOff, Qt::ScrollBarAlwaysOff);
    m_container->setFixedWidth(
        2 * (m_core->config()->property("GridSize").toInt() + 10));
    // search
