...
{
    addToolBar(
        static_cast<Qt::ToolBarArea>(m_configuration["BarPosition"].toInt()),
        m_toolBar);
    ...
    return createPage(
        m_configuration.value("path", QDir::homePath()).toString());
}
...
void FileManager::createPage(const QString &path)
{
    ...
    // create
    int index = m_stackedWidget->count() == 1
                    ? 0
                    : m_stackedWidget->currentIndex() + 1;
    m_pageWidgets.insert(
        index, new QuadroWidget(this, m_core->config()->gridSize(), path));
    ...
    QFileInfoList entries = m_core->filemanager()->directoryEntries(
        path, m_configuration["ShowHidden"].toBool());
    for (auto entry : entries) {
        QWidget *wItem = new FileIconWidget(
            entry,
            m_core->filemanager()->iconByFileName(entry.absoluteFilePath()),
        ...
        connect(wItem, SIGNAL(openFile(const QFileInfo)), this,
                SLOT(openEntry(const QFileInfo)));
...
bool FileManager::eventFilter(QObject *object, QEvent *event)
{
    if ((event->type() == QEvent::KeyPress)
        ...
        if ((static_cast<QKeyEvent *>(event)->key() == Qt::Key_Backspace)
            ...
            loadPage(QString("%1/..").arg(
                static_cast<QuadroWidget *>(m_stackedWidget->currentWidget())
                    ->title()));
        } else {
...
void FileManager::showSearchResults(const QString &search)
{
    ...
    // add items
    QFileInfoList entries = m_core->filemanager()->entriesBySubstr(
        m_pageWidgets.at(m_searchIndex)->title(), search,
        m_configuration["ShowHidden"].toBool());
    ...
    for (auto &entry : entries) {
        ...
        connect(wItem, SIGNAL(openDirInNewTab(const QFileInfo)), this,
                SLOT(openEntryInNewTab(const QFileInfo)));
        if (++count >= m_configuration["SearchResult"].toInt()) {
...
void FileManager::createActions()
{
    connect(m_toolBar, SIGNAL(actionTriggered(QAction *)), this,
            SLOT(changePageByAction(QAction *)));
...
void FileManager::updateToolbar()
{
    for (int i = 0; i < m_pageButtons.count(); i++)
        m_pageButtons[i]->setText(
            QFileInfo(m_pageWidgets.at(i)->title()).fileName());

    m_toolBar->setVisible(m_configuration["ShowToolbar"].toBool()
