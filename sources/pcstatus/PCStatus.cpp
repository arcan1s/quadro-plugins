...
void PCStatus::update()
{
    ...
    float down = m_helper->updateDownStats(
        networkDevice, m_configuration[QString("Update")].toInt());
    float up = m_helper->updateUpStats(
        networkDevice, m_configuration[QString("Update")].toInt());
    ...
    if (m_configuration[QString("ShowNet")].toBool())
        dataStats.append(
            tr("Network speed: %1/%2").arg(down, 4, 'f', 0).arg(up, 4, 'f', 0));
