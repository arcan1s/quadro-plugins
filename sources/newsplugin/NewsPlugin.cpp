...
{
    DBusOperations::sendRequestToUi("RunWebContainer",
                                    {m_helper->news().url, true});
}
...
void NewsPlugin::readSettings(const QString &configPath)
{
    ...
    delete m_helper;
    m_helper = nullptr;
    m_helper = new NewsPluginHelper(this, m_configuration["Provider"].toInt(),
                                    m_configuration["Type"].toString(),
                                    m_configuration["Retrieve"].toInt());
}
...
{
    return m_configuration["Update"].toInt();
}
