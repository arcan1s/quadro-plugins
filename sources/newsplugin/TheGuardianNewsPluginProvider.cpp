...
TheGuardianNewsPluginProvider::TheGuardianNewsPluginProvider(
    QObject *parent, const QString &type)
...
void TheGuardianNewsPluginProvider::replyReceived(QNetworkReply *reply)
{
    ...
    QVariantList jsonNews
        = jsonDoc.toVariant().toMap()["response"].toMap()["results"].toList();
    m_metadata.clear();

    for (auto &news : jsonNews) {
        NewsPluginMetadata singleData;
        singleData.image = QString();
        singleData.text = news.toMap()["webTitle"].toString();
        singleData.url = news.toMap()["webUrl"].toString();
